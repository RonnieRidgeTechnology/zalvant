...
<main class="main-content">
    ...
    <div class="formContainer"
        ...
        <form action="{{ $formAction }}" method="POST" enctype="multipart/form-data">
            ...
            <!-- Service Selection (Required, No Language Tabs) -->
            <div class="form-group" style="margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #e5e7eb;">
                <label class="form-label">Service <span style="color: #dc2626;">*</span></label>
                <select name="service_id" class="form-control" required>
                    <option value="">-- Select Service --</option>
                    @foreach($services as $service)
                        <option value="{{ $service->id }}" {{ old('service_id', $landingPage->service_id) == $service->id ? 'selected' : '' }}>
                            {{ $service->name }}
                        </option>
                    @endforeach
                </select>
