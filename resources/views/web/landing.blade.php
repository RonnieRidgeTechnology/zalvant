...
.borders5 img{
   ...
   .zalvant-form-section {
    ...
    .heroActionA::before, .heroActionA::after {
        content: "";
        position: absolute;
        width: 540px;
        height: 540px;
    ...
    .heroActionA::before { background: #005fde; top: -160px; left: -120px; }
    .heroActionA::after  { background: #005fde; bottom: -180px; right: -120px; }
...
;
    ...
    .eyebrowActionA .dotActionA { width: 8px; height: 8px; border-radius: 50%; background: #004ab7; box-shadow: 0 0 0 4px rgba(0,74,183,.25); }
    ...
    .btnActionA svg { width: 18px; height: 18px; }
    .btnActionA:active { transform: translateY(1px); }
    ...
    .btnActionA:hover .shineActionA { transform: translateX(120%); }
    ...
    .btnPrimaryActionA:hover { box-shadow: 0 16px 36px rgba(0,0,0,.35); }
    ...
    .btnGhostActionA:hover { background: rgba(255,255,255,.10); border-color: rgba(255,255,255,.45); }
    ...
    @media (max-width: 520px) {
        .cardActionA { padding: 28px; border-radius: 16px; }
    }
</style>
        ...
        <form class="zalvant-dark-form" id="zalvantContactForm" autocomplete="off" action={{route('landing.submit')}} method="post">
          ...
          <div class="zalvant-form-group">
            <label for="name">
              ...
              <span id="zalvant-label-name">{{ $formLabels?->getLocalizedNameLabel() ?? 'Naam' }}</span>
              <span class="zalvant-form-required">*</span>
            </label>
            <input type="text" id="name" name="name" placeholder="{{ $formLabels?->getLocalizedNamePlaceholder() ?? 'Jouw naam' }}" required>
          </div>
          <div class="zalvant-form-group">
            <label for="email">
              ...
              <span id="zalvant-label-email">{{ $formLabels?->getLocalizedEmailLabel() ?? 'E-mail' }}</span>
              <span class="zalvant-form-required">*</span>
            </label>
            <input type="email" id="email" name="email" placeholder="{{ $formLabels?->getLocalizedEmailPlaceholder() ?? 'E-mail' }}" required>
          </div>
          <div class="zalvant-form-group">
            <label for="phone">
              <svg width="20" height="20" fill="none" stroke="#005BD7" stroke-width="2" viewBox="0 0 24 24">
              ...
              <span id="zalvant-label-phone">{{ $formLabels?->getLocalizedPhoneLabel() ?? 'Telefoon' }}</span>
            </label>
            <input type="tel" id="phone" name="phone" placeholder="{{ $formLabels?->getLocalizedPhonePlaceholder() ?? 'Telefoonnummer' }}">
          </div>
          <div class="zalvant-form-group">
            <label for="service">
              <svg width="20" height="20" fill="none" stroke="#005BD7" stroke-width="2" viewBox="0 0 24 24">
              ...
              <span id="zalvant-label-service">{{ $formLabels?->getLocalizedServiceLabel() ?? 'Dienst geïnteresseerd in' }}</span>
              <span class="zalvant-form-required">*</span>
            </label>
            <select id="service" name="service_id" required>
              <option value="" disabled selected>{{ $formLabels?->getLocalizedServicePlaceholder() ?? 'Selecteer een dienst' }}</option>
              <option>{{ __('web.services_cards.digital_marketing') }}</option>
              <option>{{ __('web.services_cards.web_development') }}</option>
              <option>{{ __('web.services_cards.mobile_app') }}</option>
              <option>SEO</option>
...
<div class="faq-section">
 ...
 <script>
        (function () {
            ...
            const formatterCompact = Intl.NumberFormat(undefined, { notation: 'compact' });
            const formatterGrouped = Intl.NumberFormat(undefined);

            function format(value, mode) {
                switch (mode) {
                    case 'compact-plus': return formatterCompact.format(value) + '+';
                    case 'grouped': return formatterGrouped.format(value);
                    case 'plus': return formatterGrouped.format(value) + '+';
            ...
            function animateCount(el) {
                ...
                function frame(now) {
                    ...
                    if (progress < 1) requestAnimationFrame(frame); else el.textContent = format(target, mode);
                }
