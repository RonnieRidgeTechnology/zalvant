...
<div class="banner">
    <div class="banner-header">
        ...
        <h1 class="banner-title">
            @if(isset($landingPage) && $landingPage)
                {{ $landingPage->getLocalized('second_title', $locale ?? 'nl') }}
            @else
                {{ $banner?->getLocalizedSecondTitle() ?? 'Technology that helps your business move forward' }}
            @endif
        </h1>
        <p class="banner-des">
            @if(isset($landingPage) && $landingPage)
                {{ $landingPage->getLocalized('second_desc', $locale ?? 'nl') }}
            @else
                {{ $banner?->getLocalizedSecondDesc() ?? 'Transform your ideas into reality with our expert website development, mobile app development, SEO, digital marketing, and IT solutions.' }}
            @endif
        </p>
    ...
    <section class="about-section">
        ...
        <div class="description">
            <h1>
                @if(isset($landingPage) && $landingPage)
                    {{ $landingPage->getLocalized('third_title', $locale ?? 'nl') }}
                @else
                    {{ $homeupdate->getLocalizedOfferTitle() ?? 'What We Offer' }}
                @endif
            </h1>
            <p>
                @if(isset($landingPage) && $landingPage)
                    {{ $landingPage->getLocalized('third_desc', $locale ?? 'nl') }}
                @else
                    {{ $homeupdate->getLocalizedOfferDesc() ?? 'We offer a wide range of services to help your business grow and succeed in the digital age.' }}
                @endif
            </p>
        </div>

        {{-- Right Column: Feature Blocks from Landing Page or Services --}}
        <div class="about-cards-container" style="flex: 2;">
            @if(isset($landingPage) && $landingPage && $landingPage->feature_blocks && count($landingPage->feature_blocks) > 0)
                {{-- Display Feature Blocks from Landing Page --}}
                @php
                    $features = collect($landingPage->getLocalizedFeatures($locale ?? 'nl'));
                @endphp
                @foreach ($features->chunk(2) as $featureRow)
                <div class="service-row" style="display: flex; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div class="about-card" style="flex: 1;">
                        @if($feature['icon'])
                            <div class="icon">
                                <img src="{{ asset($feature['icon']) }}" alt="{{ $feature['title'] ?? 'Feature Icon' }}">
                            </div>
                        @endif
                        @if($feature['title'])
                            <h4>{{ $feature['title'] }}</h4>
    ...
    @php
    $hasAiDevelopmentService = false;
    if (isset($landingPage) && $landingPage && $landingPage->services) {
        $hasAiDevelopmentService = $landingPage->services->contains('type', 'ai-development');
    }
@endphp

@if($hasAiDevelopmentService)
